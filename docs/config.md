...
`subsessions` controls whether agents receive the tracked-subsession tools: `spawn_subsession`, `list_subsessions`, `check_subsession`, `read_subsession`, `send_subsession_message`, and `yield_to_subsessions`. It defaults to `true` and also requires `spawnSessions` to be enabled.
