...
PI WEB admits at most four concurrent runs per plugin. Admission/start failures reject `run()`; the completion settles as `completed`, `cancelled`, or `failed`, with failure text bounded to 4 KiB of UTF-8. Plugin lifetime revocation stops new admissions, aborts all runs owned by that plugin, reclaims any session that finishes startup after cancellation, and boundedly closes runtimes while preserving their transcripts before capability cleanup finishes.
...
`bundled-only` safe start excludes external server plugins before their capabilities are created. `none` imports no server plugins and creates no plugin capability consumers; kernel folder workspaces and recovery/diagnostic surfaces remain available, while required Terminal and Terminal-backed workflows do not. During ordinary shutdown PI WEB closes peer ingress and aborts plugin lifetimes while host session authority is still available for owned-run cleanup, then disposes remaining sessions. A missing or failed optional capability disables its dependent plugins with attributed diagnostics; unrelated kernel and plugin surfaces continue.
