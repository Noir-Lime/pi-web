...
function findCopyDiagnosticsAction() {
  const action = plugin.activate({ apiVersion: 3, pluginId: "info", runtimePluginId: "info", html, svg }).contributions.actions?.find((candidate) => candidate.id === "copy-diagnostics");
