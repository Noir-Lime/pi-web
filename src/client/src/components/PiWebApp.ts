...
export class PiWebApp extends LitElement {
  ...
  private renderContextBar() {
    ...
    return html`
      <app-context-bar
        ...
        .hiddenActiveDestination=${this.availableNavigationTabs().some((tab) => tab.id === this.effectiveMainView())}
        .onShowActions=${this.navigationActions.showActions}
      ></app-context-bar>
    `;
  }
  ...
  private renderMobileMainTabs() {
    ...
    const selectedView = this.effectiveMainView(panels);
    return html`
      <app-mobile-main-tabs
        .tabs=${pinnedTabs}
        .hiddenActiveDestination=${availableTabs.some((tab) => tab.id === selectedView) && !pinnedTabs.some((tab) => tab.id === selectedView)}
        .onShowNavigation=${this.showNavigation}
    ...
    return html`
      <div class=${this.panelCollapse.shellClass(mainView)} style=${this.panelResize.shellStyle({ navigation: this.resizablePanelConstraints("navigation"), workspace: this.resizablePanelConstraints("workspace") })}>
        ...
        ${this.navigationDialogOpen ? html`<navigation-dialog
          ...
          .selectedView=${this.appShell.isDesktopSideBySideLayout ? this.effectiveWorkspaceTool() : mainView}
